...
> Source: derived from the repository (`CLAUDE.md`, the Capacitor app, `docs/native/`) and the
> design + native migration brief. Facts marked *(inferred)* were not confirmed in an interview and
> must be confirmed by the product owner before being treated as binding.
...
- Business logic lives in the FastAPI backend; native clients present, navigate and integrate with
  ...
  `docs/native/CURRENT_STATE_AUDIT.md` §5).
- Full feature inventory: `docs/native/PARITY_MATRIX.md`.
