...
export default function StrategyBasic({ plan = "basic" }){
  ...
  return <section className={`strategy-page plan-view-${plan}`}>
    <div className="hero"><span className="strategy-plan-kicker">{vip?<><Crown size={15}/> VIP</>:<><Sparkles size={15}/> BASIC</>}</span><h1>{vip?"Dirección financiera VIP":"Estrategia Basic"}</h1><p>{vip?"JARVIS coordina tus prioridades, deuda, seguridad, metas y margen personal.":"Una estrategia matemática construida con tus datos. Si falta información, JARVIS te lo dice en vez de inventarla."}</p></div>
    ...
    {d.next_paycheck&&<div className="panel paycheck-card"><div className="strategy-section-title"><PiggyBank size={18}/><h3>Próximo ingreso</h3></div><p>Con tu frecuencia de pago actual, JARVIS estima {money(d.next_paycheck.estimated_paycheck)} por pago y propone separar:</p><div className="allocation-list">{d.next_paycheck.envelopes.map((a,i)=><div className="allocation-row" key={`pay-${a.bucket}-${i}`}><span>{a.label}</span><strong>{money(a.amount)}</strong></div>)}</div>{d.next_paycheck.unassigned>0&&<small>Sin asignar: {money(d.next_paycheck.unassigned)}</small>}</div>}
