...
def get_business_center():
    ...
    with get_connection() as conn:
        _ensure_tables(conn)
        businesses = conn.execute("SELECT * FROM business_projects WHERE user_id=%s ORDER BY status, name", (user_id,)).fetchall()
        movements = conn.execute("SELECT * FROM business_movements WHERE user_id=%s ORDER BY movement_date DESC,id DESC LIMIT 200", (user_id,)).fetchall()
        totals = conn.execute("""
          SELECT
          ...
          FROM business_movements WHERE user_id=%s
        """, (user_id,)).fetchone()
        conn.commit()
...
def create_business(request: BusinessRequest):
    ...
    with get_connection() as conn:
        _ensure_tables(conn)
        row = conn.execute("""INSERT INTO business_projects(user_id,name,description,ownership_pct)
          VALUES(%s,%s,%s,%s) RETURNING *""", (user_id, request.name.strip(), request.description, request.ownership_pct)).fetchone()
        conn.commit()
...
def add_business_movement(request: BusinessMovementRequest):
    ...
    with get_connection() as conn:
        _ensure_tables(conn)
        business = conn.execute("SELECT * FROM business_projects WHERE id=%s AND user_id=%s", (request.business_id,user_id)).fetchone()
        ...
        if kind in {'income','expense'}:
            ...
            tx = conn.execute("""INSERT INTO transactions(user_id,transaction_date,description,amount,transaction_type,category,source,notes)
              VALUES(%s,%s,%s,%s,%s,%s,'business_center',%s) RETURNING id""",
              (user_id, str(movement_date), request.description, request.amount, tx_type, category,
               f"business_id:{request.business_id}; business:{business['name']}")).fetchone()
            transaction_id = tx['id']
        row = conn.execute("""INSERT INTO business_movements(user_id,business_id,movement_date,movement_type,amount,description,category,transaction_id)
          VALUES(%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
          (user_id,request.business_id,movement_date,kind,request.amount,request.description,request.category,transaction_id)).fetchone()
        conn.commit()
    return serialize_row(row)
