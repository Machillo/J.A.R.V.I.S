...
def _parse_time(text: str) -> str:
    match = re.search(r"(?:a las|alas|@)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm|a\.m\.|p\.m\.)?", text, re.I)
    if not match:
        return "09:00"
...
def extract_event_date(text: str) -> Optional[str]:
    ...
    if "pasado mañana" in text_l or "pasado manana" in text_l:
    ...
    numeric = re.search(r"\b(\d{1,2})[/-](\d{1,2})(?:[/-](20\d{2}|\d{2}))?\b", text_l)
    if numeric:
        d, m, y_raw = numeric.groups()
        y = int(y_raw) if y_raw else today.year
        if y < 100:
            y += 2000
        event_day = date(y, int(m), int(d))
        if event_day < today and not y_raw:
...
def extract_title(text: str) -> str:
    cleaned = re.sub(r"^\s*(jarvis|karvis|jervis|jarbis)[,\s]*", "", text, flags=re.I)
    cleaned = re.sub(
        r"\b(recordame|recuerdame|recu[eé]rdame|agenda|agend[aá]|agendame|tengo|crear|crea|guardar|guarda|compromiso|actividad|evento|cita|reunion|reunión)\b",
        "",
        cleaned,
        flags=re.I,
    )
    ...
    cleaned = re.sub(r"\ba las\s*\d{1,2}(:\d{2})?\s*(am|pm|a\.m\.|p\.m\.)?\b", "", cleaned, flags=re.I)
