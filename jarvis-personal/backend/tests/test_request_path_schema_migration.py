...
def _with_bodies(statements):
    """Statements plus the statements inside DO/function bodies ($$ ... $$)."""
    for statement in statements:
        yield statement
        for body in re.findall(r"\$([A-Za-z_]*)\$(.*?)\$\1\$", statement, re.S):
            yield from (part.strip() for part in re.split(r"[;\n]", body[1]) if part.strip())


def test_the_migration_never_destroys_or_rewrites():
    for statement in _with_bodies(apply_migration.statements(MIGRATION.read_text(encoding="utf-8"))):
        ...
        assert not re.search(r"\b(DROP\s+COLUMN|ALTER\s+COLUMN|DROP\s+CONSTRAINT|RENAME)\b", upper), statement
...
def test_no_migration_still_creates_a_gated_table_outside_this_one():
    ...
    for path in [*(DATABASE / "migrations").glob("*.sql"), DATABASE / "schema.sql"]:
        if path == MIGRATION:
            continue
        for statement in apply_migration.statements(path.read_text(encoding="utf-8")):
            created = re.search(r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:\"?public\"?\.)?\"?([a-z_]+)\"?",
                                statement, re.I)
            assert not (created and created.group(1).lower() in gated), path.name
