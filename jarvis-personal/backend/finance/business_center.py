...
def create_business(request: BusinessRequest):
    ...
    with get_connection() as conn:
        _ensure_tables(conn)
        row = conn.execute("""INSERT INTO business_projects(workspace_id,name,description,ownership_pct)
          VALUES(%s,%s,%s,%s) RETURNING *""", (workspace_id, request.name.strip(), request.description, request.ownership_pct)).fetchone()
        conn.commit()
...
def add_business_movement(request: BusinessMovementRequest):
    ...
    with get_connection() as conn:
        ...
        if kind in {'income','expense'}:
            ...
            tx = conn.execute("""INSERT INTO transactions(workspace_id,transaction_date,description,amount,transaction_type,category,source,notes)
              VALUES(%s,%s,%s,%s,%s,%s,'business_center',%s) RETURNING id""",
              (workspace_id, str(movement_date), request.description, request.amount, tx_type, category,
               f"business_id:{request.business_id}; business:{business['name']}")).fetchone()
            transaction_id = tx['id']
        row = conn.execute("""INSERT INTO business_movements(workspace_id,business_id,movement_date,movement_type,amount,description,category,transaction_id)
          VALUES(%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
          (workspace_id,request.business_id,movement_date,kind,request.amount,request.description,request.category,transaction_id)).fetchone()
        conn.commit()
    return serialize_row(row)
