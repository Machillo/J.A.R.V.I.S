...
DINCR_EMAIL = os.getenv("DINCR_ISOLATION_TEST_EMAIL", "")
...
def main():
    ...
    dincr = resolve_context(DINCR_EMAIL)

    if personal["account_id"] == dincr["account_id"]:
        fail("Personal y DINCR resolvieron al mismo account_id.")
    if personal["workspace_id"] == dincr["workspace_id"]:
    ...
    f = create_sentinels(dincr, f"F_{marker}")
    ...
    try:
        ...
        print("\nDINCR <-> PERSONAL RUNTIME ISOLATION")
        print("=" * 52)
        print(f"Personal account : {personal['account_id']}")
        print(f"Personal workspace: {personal['workspace_id']}")
        print(f"DINCR account    : {dincr['account_id']}")
        print(f"DINCR workspace  : {dincr['workspace_id']}")
        print("-" * 52)
