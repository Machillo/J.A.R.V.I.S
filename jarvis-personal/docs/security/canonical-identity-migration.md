...
| Notification join `allowed_users.id = notification_subscriptions.user_id` (Owner system push; calendar and fixed-expense reminders read by workspace since Phase C) | `notifications/service.py` | LEGACY COMPATIBILITY → canonical Owner identity (explicit Owner role) |
...
| Id-space mixing: `integrations/ibkr_readonly.py::_owner_identity` (`users.id = legacy_allowed_user_id` join), dead `email_monitor::_owner_user_id` | as listed | UNKNOWN / NEEDS REVIEW (fix before Phase B) |
