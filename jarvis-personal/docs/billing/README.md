...
- **A store subscription** in `store_subscriptions`, verified server-side, while it is `trialing` (until `trial_ends_at`), `active` or `grace_period` (until `current_period_end`). See `product_ops.service.has_store_entitlement`.
...
- **Rollback:** `database/rollback/20260926120000_retire_offstore_billing_rollback.sql` recreates the three tables empty (plus the configuration row). It is only needed to redeploy older code.
