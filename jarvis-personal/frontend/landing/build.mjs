...
function layout(path, title, content, { noindex = false, desc = description, schema = false } = {}) {
  ...
  return `<!doctype html><html lang="es-CR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)} | DINCR</title><meta name="description" content="${escape(desc)}">${noindex || !base ? '<meta name="robots" content="noindex,follow">' : ''}${canonical}<meta name="theme-color" content="#0A1220" media="(prefers-color-scheme: dark)"><meta name="theme-color" content="#F5F7FA" media="(prefers-color-scheme: light)"><meta name="color-scheme" content="dark light"><meta property="og:type" content="website"><meta property="og:locale" content="es_CR"><meta property="og:site_name" content="DINCR"><meta property="og:title" content="${escape(title)} | DINCR"><meta property="og:description" content="${escape(desc)}">${image}<meta name="twitter:card" content="summary"><meta name="twitter:title" content="${escape(title)} | DINCR"><meta name="twitter:description" content="${escape(desc)}"><link rel="icon" href="/favicon-32.png" sizes="32x32" type="image/png"><link rel="apple-touch-icon" href="/apple-touch-icon.png"><link rel="stylesheet" href="/style.css?v=${cssVersion}">${ld}</head><body><a class="skip" href="#contenido">Saltar al contenido</a>${header}<main id="contenido">${content}</main>${footer}</body></html>`;
}
...
const heroVisual = cfg.heroScreenshot
  ...
  : `<figure class="hero-visual"><div class="illus" aria-hidden="true"><div class="illus-card"><span class="illus-label">Aviso de tu banco detectado</span><div class="illus-row"><strong>Supermercado</strong><span class="illus-amount">−₡12.500</span></div><div class="illus-actions"><span class="chip ok">Confirmar</span><span class="chip">Corregir</span><span class="chip">Descartar</span></div></div><div class="illus-card illus-card-soft"><span class="illus-label">Presupuesto de este mes</span><div class="bar"><i style="width:62%"></i></div><span class="illus-note">62 % usado</span></div></div><figcaption>Ilustración con datos de ejemplo.</figcaption></figure>`;
...
const home = `<section class="hero"><div class="wrap hero-grid"><div class="hero-copy"><h1>Tus finanzas en orden, con menos trabajo manual.</h1><p class="lead">DINCR es la app de finanzas personales para Costa Rica: reúne tus ingresos, gastos, deudas y metas y te muestra con claridad dónde estás y qué sigue.</p><p class="lead-sub">Con VIP, detecta los avisos de tu banco en tu correo para que solo tengás que confirmarlos.</p><div class="actions"><a class="button" href="#planes">Ver planes</a><a class="button button-ghost" href="#como-funciona">Cómo funciona</a></div>${availability}</div>${heroVisual}</div></section>
<section class="section problem"><div class="wrap"><h2>Tu dinero está en todos lados. Tu panorama, en ninguno.</h2><ul class="pain"><li>Los avisos del banco se pierden en el correo.</li><li>Cada deuda tiene su cuota y su fecha.</li><li>Sin registro, cuesta saber cuánto podés gastar.</li></ul><p class="section-lead">DINCR lo junta en un solo lugar y lo mantiene al día.</p></div></section>
<section class="section band" id="como-funciona"><div class="wrap"><h2>Vos conservás el control.</h2><ol class="steps">${steps.map(([h, p]) => `<li><h3>${h}</h3><p>${p}</p></li>`).join('')}</ol></div></section>
<section class="section" id="funciones"><div class="wrap"><h2>Lo que necesitás para decidir mejor.</h2><div class="features">${features.map(([h, p, plan]) => `<article class="feature"><span class="badge badge-${plan.toLowerCase()}">${plan}</span><h3>${h}</h3><p>${p}</p></article>`).join('')}</div><p class="note">La insignia indica el plan desde el que está disponible cada función.</p></div></section>
<section class="section band" id="privacidad"><div class="wrap split"><div><h2>Tus datos financieros son tuyos.</h2><p class="section-lead">DINCR maneja información sensible. Por eso el acceso al correo es opcional, limitado y revocable.</p><div class="inline-links"><a href="/privacidad/">Política de privacidad</a><a href="/seguridad/">Seguridad</a></div></div><ul class="checks">${privacyPoints.map(p => `<li>${p}</li>`).join('')}</ul></div></section>
<section class="section" id="planes"><div class="wrap"><h2>Empezá gratis. Crecé cuando lo necesités.</h2>${planCards(3)}${promo}<a class="text-link" href="/precios/">Comparar planes en detalle</a></div></section>
<section class="section band"><div class="wrap narrow"><h2>Lo esencial, sin letra pequeña.</h2><div class="faqs">${faq}</div></div></section>
...
const mark = v => v ? '<span class="yes" aria-hidden="true"></span><span class="sr-only">Incluido</span>' : '<span class="muted" aria-hidden="true">—</span><span class="sr-only">No incluido</span>';
...
const pages = {
  ...
  '/descargar/': layout('/descargar/', 'Descargar DINCR', `<section class="section page"><div class="wrap"><h1>DINCR para Android y iPhone</h1><p class="section-lead">Si entrás con la misma cuenta en los dos sistemas, ves la misma información y el mismo plan.</p>${availability}<div class="platforms">${platforms}</div></div></section>`, { noindex: !cfg.googlePlayUrl && !cfg.appStoreUrl }),
};
