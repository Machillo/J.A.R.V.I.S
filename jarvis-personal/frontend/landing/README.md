...
The canonical origin in `landing/config.json` is `https://dincr.com`. Store links remain disabled until official URLs are supplied. The public contact is `soporte@dincr.com`; the mailbox itself still needs to be configured.
...
The legal pages are rendered from `src/pages/PublicInfoPage.jsx` at build time. VIP's monthly launch price is ₡4.990 and public support is `soporte@dincr.com`. The mailbox provider has not yet been configured; do not publish before it receives mail. The review block is disabled until authentic, authorized data is configured.
