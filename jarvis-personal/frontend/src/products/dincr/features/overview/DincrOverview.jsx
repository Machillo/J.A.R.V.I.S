...
function MetricCard({ label, value, detail, tone = "neutral", icon: Icon }) {
  return (
    <article className={`dincr-overview-metric tone-${tone}`}>
      <span className="dincr-overview-metric-icon"><Icon size={19} /></span>
      <small>{label}</small>
      <strong>{value}</strong>
...
function BasicDashboard({ data, planning, onNavigate }) {
  ...
  return <section className="dincr-basic-dashboard">
    <article className="dincr-basic-hero-card">
    ...
    <article className="dincr-basic-quick-card">
    ...
    <article className="dincr-basic-upcoming">
    ...
    <article className="dincr-basic-plan-progress">
...
export default function DincrOverview({ user, plan = "free", onNavigate }) {
  ...
  if (error) return <div className="mobile-panel dincr-overview-state error">{error}</div>;
  if (!data) return <div className="mobile-panel dincr-overview-state">{tx("Preparando tu resumen...", "Preparing your overview...")}</div>;

  if (plan === "basic") return <BasicDashboard data={data} planning={planning} onNavigate={onNavigate}/>;

  return (
    <section className={`dincr-overview dashboard-${plan}`}>
      {advanced ? <>
        <header className="dincr-overview-hero">
        ...
        <div className="dincr-overview-metrics">
        ...
        <small className="dincr-free-plan-label">{tx("Gratis", "Free")}</small>
        <article className="dincr-free-available">
        ...
        <article className="dincr-free-kpis">
      ...
      <article className="mobile-panel dincr-overview-chart">
        ...
        {computed.history.length ? computed.history.map((row) => (
          <div className="dincr-overview-chart-row" key={row.month}>
        ...
        )) : <p className="dincr-overview-empty">{tx("Agregá tus primeros movimientos para ver la comparación mensual.", "Add your first transactions to see the monthly comparison.")}</p>}
        <footer><span><i className="income" /> {tx("Ingresos", "Income")}</span><span><i className="expense" /> {tx("Gastos", "Expenses")}</span></footer>
      </article>

      <article className="mobile-panel dincr-overview-categories">
        ...
        )) : <p className="dincr-overview-empty">{tx("Cuando registrés gastos, los agruparemos acá automáticamente.", "When you record expenses, we will group them here automatically.")}</p>}
      </article>

      {plan === "free" && <button className="dincr-overview-action" onClick={() => onNavigate?.("monthly")}><span><strong>{tx("Ver resumen mensual", "View monthly summary")}</strong><small>{tx("Revisá tus números con más detalle.", "Review your numbers in more detail.")}</small></span><ChevronRight /></button>}
      {plan === "vip" && <button className="dincr-overview-action vip" onClick={() => onNavigate?.("strategy")}><Crown /><span><strong>{tx("Abrir Dirección VIP", "Open VIP Direction")}</strong><small>{tx("Tu estrategia financiera completa.", "Your complete financial strategy.")}</small></span><ChevronRight /></button>}
    </section>
  );
}
