...
export function createFinvaFeatureRegistry({ user, plan, navigate, onUserChange, onLogout, featureFlags }) {
  ...
  return {
    ...
    gmail: plan === "vip" ? gated("gmail_automation", <GmailAutomation onNavigate={navigate} />) : <SettingsPage user={user} onUserChange={onUserChange} onLogout={onLogout} />,
    // Same gate as the mail review it groups by bank: bank emails are a VIP feature.
    accounts: plan === "vip" ? gated("gmail_automation", <GmailAutomation view="accounts" onNavigate={navigate} />) : <SettingsPage user={user} onUserChange={onUserChange} onLogout={onLogout} />,
    ...
    monthly: <MonthlySummary plan={plan} />,
    feedback: <Feedback />,
