...
const STATE_KEY = "finvaNavigation";
const EDGE_START_PX = 28;
const SWIPE_DISTANCE_PX = 84;

export function useFinvaBackHandler(handler, enabled = true) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("finva:native-back", consume);
    return () => window.removeEventListener("finva:native-back", consume);
  }, [enabled]);
}
...
export default function useFinvaNavigation(initialPage = "overview") {
  ...
  const back = useCallback(() => {
    const event = new CustomEvent("finva:native-back", { cancelable: true });
