...
export default function Businesses(){
 ...
 if(!data&&!error)return <section className="business-page"><div className="hud-panel"><RefreshCw className="spin"/> {tx("Cargando negocios...", "Loading businesses...")}</div></section>;
 const d=data||{};
 return <section className="business-page investments-page">
  <div className="investment-hero hud-panel"><div><span className="strategy-eyebrow">WEALTH · BUSINESS</span><h2>{tx("Negocios", "Businesses")}</h2><p>Medí cuánto aportás, cuánto producen tus proyectos y cuánto ingreso no laboral generan.</p></div><button className="strategy-refresh-btn" onClick={load}><RefreshCw size={17}/> {tx("Actualizar", "Refresh")}</button></div>
  ...
  <div className="hud-panel"><div className="panel-heading"><div><span className="strategy-eyebrow">PORTAFOLIO DE NEGOCIOS</span><h3>{tx("Mis negocios", "My businesses")}</h3></div></div>{!d.businesses?.length?<p className="muted-text">Todavía no hay negocios. Podés crear DINCR, el futuro bot o una participación en otro emprendimiento.</p>:<div className="business-list">{d.businesses.map(b=>{const ms=(d.movements||[]).filter(m=>Number(m.business_id)===Number(b.id));const inc=ms.filter(m=>m.movement_type==='income').reduce((a,m)=>a+Number(m.amount),0),exp=ms.filter(m=>m.movement_type==='expense').reduce((a,m)=>a+Number(m.amount),0);return <div className="business-card" key={b.id}><div><strong>{b.name}</strong><small>{Number(b.ownership_pct||100).toFixed(0)}% participación</small></div><div><span>Ingresos {crc(inc)}</span><span>Gastos {crc(exp)}</span><b>{crc(inc-exp)}</b></div></div>})}</div>}</div>
 </section>
}
