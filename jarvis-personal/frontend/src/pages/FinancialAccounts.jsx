...
const candidateDate = (item) => {
  ...
  return value ? new Date(value).toLocaleDateString("es-CR", { day: "numeric", month: "short" }) : "";
};
...
export default function FinancialAccounts({ onFinanceChanged }) {
  ...
  const load = async () => {
    ...
    if (accountsResult.status === "fulfilled") {
      ...
      setState((old) => ({ ...old, loading: false, error: accountsResult.reason?.message || "No pude cargar las cuentas." }));
    }
    if (emailResult.status === "fulfilled") {
      setEmailState({ loading: false, items: emailResult.value?.items || [], error: "" });
    } else {
      setEmailState({ loading: false, items: [], error: emailResult.reason?.message || "No pude leer los movimientos bancarios." });
    }
  };
  ...
  const remove = async (id) => {
    if (!window.confirm("¿Ocultar esta cuenta financiera?")) return;
  ...
  return <section className="financial-accounts-page jarvis-v2-screen financial-accounts-v2">
    <div className="bank-accounts-heading">
      <div><span className="strategy-eyebrow">EMAIL BANKING</span><h2>Cuentas</h2><p>JARVIS reconoce la institución desde los correos que ya procesa. Revisá cada movimiento antes de incorporarlo a finanzas.</p></div>
      <button className="bank-add-button" type="button" onClick={() => setEditing(!editing)} aria-label="Agregar cuenta"><Plus size={22}/></button>
    </div>

    <section className="bank-connections-panel">
      <div className="bank-section-title"><div><strong>Bancos detectados</strong><small>BAC y MultiMoney · versión 1</small></div><MailSearch size={20}/></div>
      {emailState.loading ? <div className="bank-empty">Reconociendo bancos en tus correos...</div> :
        emailState.error ? <div className="bank-empty">{emailState.error}</div> :
        detectedBanks.length === 0 ? <div className="bank-empty">Todavía no encontré correos de BAC o MultiMoney.</div> :
            ...
            return <div className={`bank-detected-card ${open ? "open" : ""}`} key={bank.id}>
              <button className="bank-detected-summary" type="button" onClick={() => setSelectedBank(open ? null : bank.id)}>
                ...
                <span className="bank-detected-copy">
                  ...
                  <small>{bank.last4.length ? bank.last4.map((last4) => `•••• ${last4}`).join(" · ") : "Cuenta detectada por correo"}</small>
                  <small>{bank.items.length} movimientos detectados · {bank.pending.length} por revisar</small>
                </span>
                ...
                {bank.items.slice(0, 50).map((item) => {
                  ...
                  return <article className="bank-movement-row" key={item.id}>
                    <div className="bank-movement-copy">
                      <strong>{item.description || item.email_subject || "Movimiento bancario"}</strong>
                      <small>{candidateDate(item)}{item.category ? ` · ${item.category}` : ""}{item.card_last4 ? ` · •••• ${item.card_last4}` : ""}</small>
                      <small className={`bank-movement-status status-${item.status}`}>{pending ? "Por revisar" : item.status === "rejected" ? "Rechazado" : item.status === "duplicate" ? "Duplicado" : "En finanzas"}</small>
                    </div>
                    <div className="bank-movement-side">
                      ...
                      {pending && <div className="bank-review-actions">
                        <button className="approve" type="button" disabled={decisionId === item.id} onClick={() => decide(item, "confirm")} aria-label="Aprobar movimiento"><Check size={17}/></button>
                        <button className="reject" type="button" disabled={decisionId === item.id} onClick={() => decide(item, "reject")} aria-label="Rechazar movimiento"><X size={17}/></button>
                      </div>}
                    </div>
                  </article>;
                })}
    ...
    <div className="financial-account-kpis jarvis-v2-metrics"><div className="jarvis-v2-metric"><WalletCards/><span>Saldo real registrado</span><strong>{money(state.data?.total_real_balance)}</strong></div><div className="jarvis-v2-metric"><RefreshCw/><span>Diferencia por conciliar</span><strong>{money(state.reconciliation?.difference)}</strong><small>{state.reconciliation?.leak_alert?.message}</small></div></div>
