...
export default function FinancialAccounts() {
  ...
  const [form, setForm] = useState({ account_name: "", bank_name: "", account_type: "checking", account_last4: "", currency: "CRC", current_balance: "", include_in_net_worth: true });
  const load = async () => {
  ...
  const save = async (event) => {
    ...
    await saveAccountBalance({ ...form, current_balance: Number(form.current_balance || 0), source: "manual_reconciliation" });
    setEditing(false); setForm({ account_name: "", bank_name: "", account_type: "checking", account_last4: "", currency: "CRC", current_balance: "", include_in_net_worth: true });
    await load();
  };
  const edit = (item) => { if (item.read_only) return; setForm({ account_name: item.account_name, bank_name: item.bank_name || "", account_type: item.account_type || "checking", account_last4: item.account_last4 || "", currency: item.currency || "CRC", current_balance: item.calculated_balance ?? item.current_balance, include_in_net_worth: item.include_in_net_worth !== false }); setEditing(true); };
