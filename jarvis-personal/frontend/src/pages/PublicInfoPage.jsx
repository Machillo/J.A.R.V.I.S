...
function PrivacyPage() {
  return <>
    <span className="public-kicker">VERSIÓN 2026-09-25-v4 · VIGENTE DESDE EL 25 DE SEPTIEMBRE DE 2026</span>
    ...
    <section className="public-copy legal-copy">
      <h2>1. Responsable</h2><p>El responsable del tratamiento es Kenneth Andrés Alvarado Obando, Costa Rica, desarrollador y operador de DINCR. Para consultas sobre privacidad y datos personales, escribí a <a href="mailto:privacidad@dincr.com">privacidad@dincr.com</a>. Para ayuda general, escribí a <a href="mailto:soporte@dincr.com">soporte@dincr.com</a> o utilizá Reportes.</p>
      <h2>2. Datos tratados</h2><p>Podemos tratar identificación y cuenta —nombre, correo e identificadores técnicos—; información financiera ingresada por el usuario —ingresos, gastos, saldos, deudas, metas y presupuestos—; comprobantes de pago y estados de cuenta; eventos de uso, diagnósticos de errores y datos básicos del dispositivo; y, si el usuario conecta un correo, el remitente, asunto, fecha y contenido de los mensajes de remitentes financieros compatibles.</p>
      ...
      <h2>5. Proveedores y transferencias</h2><p>Para operar el servicio pueden intervenir proveedores de autenticación, base de datos, alojamiento, distribución móvil, monitoreo y correo, incluidos Supabase (autenticación y base de datos), Render (alojamiento), Google (Gmail y, en Android, Firebase Analytics y Crashlytics para métricas de uso y diagnóstico de errores), Microsoft (Outlook) y PostHog (analítica de producto anónima, sin correo, nombre ni datos financieros). DINCR no utiliza proveedores de inteligencia artificial generativa para prestar el servicio, y los datos obtenidos de Gmail y Outlook nunca se comparten con proveedores de inteligencia artificial ni de analítica (ver sección 6). Solo reciben la información necesaria para su función y pueden procesarla fuera de Costa Rica bajo sus medidas y condiciones de protección.</p>
      <h2>6. Integraciones de correo (Google y Microsoft)</h2>
      <p><strong>Acceso.</strong> La conexión es opcional y la inicia el usuario. Con el permiso de solo lectura de Gmail (<code>gmail.readonly</code>) o Outlook (<code>Mail.Read</code>), DINCR busca únicamente mensajes de remitentes financieros admitidos (avisos bancarios, estados de cuenta y órdenes patronales de la CCSS) y lee su remitente, asunto, fecha, contenido y adjuntos PDF. No lee ni guarda otros correos. De esos mensajes deriva movimientos propuestos (fecha, descripción, monto, moneda, banco y últimos cuatro dígitos de la cuenta o tarjeta), cuentas detectadas y datos de planilla para el cálculo del aguinaldo. No se guarda el cuerpo completo de los correos ni los archivos PDF.</p>
      <p><strong>Uso.</strong> Estos datos se usan solo para funciones visibles para el usuario: proponer movimientos que el usuario acepta, corrige o rechaza, mostrar sus bancos y cuentas, reconciliar estados de cuenta y calcular el aguinaldo. DINCR no envía, modifica ni borra correos.</p>
      <p><strong>Transferencia.</strong> Los datos obtenidos de Gmail y Outlook, crudos o derivados, no se venden, no se usan para publicidad, no se transfieren a intermediarios de datos (data brokers), no se usan para evaluar crédito ni para decisiones de préstamo, no se envían a servicios de inteligencia artificial y no se usan para desarrollar, mejorar ni entrenar modelos de inteligencia artificial o aprendizaje automático. Solo los procesan los proveedores que alojan el servicio (Supabase y Render) por cuenta de DINCR. Ninguna persona los lee, salvo con el consentimiento del usuario, por seguridad o cuando la ley lo exija.</p>
      <p><strong>Protección.</strong> La autorización (token de actualización) se guarda cifrada en Supabase Vault y nunca llega a la aplicación; los datos viajan por conexiones cifradas y se separan por cuenta.</p>
      <p><strong>Conservación y eliminación.</strong> El detalle técnico usado para revisar un hallazgo se elimina después de 30 días y el remitente y asunto del correo después de 90 días, cuando no haya revisiones pendientes. Los movimientos que el usuario confirma forman parte de su historial financiero hasta que los elimine o elimine su cuenta. Al desconectar el correo o eliminar la cuenta, la autorización se borra y, en Gmail, se revoca ante Google; al eliminar la cuenta también se eliminan los datos derivados. El usuario también puede revocar el acceso desde su cuenta de Google.</p>
      <p lang="en">DINCR's use and transfer of information received from Google APIs to any other app will adhere to the <a href="https://developers.google.com/terms/api-services-user-data-policy" target="_blank" rel="noreferrer">Google API Services User Data Policy</a>, including the Limited Use requirements. The use of raw or derived user data received from Workspace APIs will adhere to the Google User Data Policy, including the Limited Use requirements. Google Workspace API data, raw or derived, is not transferred to OpenAI or other generative-AI providers and is not used to develop, improve, or train generalized AI/ML models.</p>
      ...
      <h2>9. Derechos del usuario</h2><p>El usuario puede descargar una copia de sus datos y eliminar su cuenta desde la aplicación, y solicitar rectificación, actualización o revocación del consentimiento cuando proceda. También puede desconectar integraciones desde la aplicación o desde el proveedor correspondiente. La revocación no afecta tratamientos ya realizados legítimamente y puede impedir que ciertas funciones continúen.</p>
