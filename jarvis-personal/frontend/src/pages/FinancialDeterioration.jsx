...
export default function FinancialDeterioration() {
  ...
  useEffect(() => { load(); }, []);
  if (state.loading) return <section className="hud-panel">{tx("Analizando tendencia financiera...", "Analyzing financial trend...")}</section>;
  ...
  return <section className="deterioration-page"><div className="hud-panel deterioration-hero"><div><span className="strategy-eyebrow">JARVIS 11 · EARLY WARNING</span><h2>{tx("Deterioro financiero", "Financial deterioration")}</h2><p>Comparación de periodos para detectar señales negativas antes de que se conviertan en una crisis.</p></div><button className="strategy-refresh-btn" onClick={load}><RefreshCw size={17}/> {tx("Actualizar", "Refresh")}</button><div className={`deterioration-status ${data.health}`}><span>{data.health === "stable" ? <CheckCircle2/> : <ShieldAlert/>} Estado general</span><strong>{data.health === "deteriorating" ? "En deterioro" : data.health === "watch" ? "Para vigilar" : "Estable"}</strong><small>Periodo: {data.period?.current || "actual"}</small></div></div>
    <div className="deterioration-kpis"><article className="hud-card"><WalletCards/><span>{tx("Liquidez disponible", "Available liquidity")}</span><strong>{crc(ctx.liquidity_available)}</strong></article><article className="hud-card"><ShieldAlert/><span>Cobertura Salvavidas</span><strong>{Number(ctx.salvavidas_coverage_months || 0).toFixed(2)} meses</strong></article><article className="hud-card"><TrendingDown/><span>{tx("Deuda pendiente", "Outstanding debt")}</span><strong>{crc(ctx.debt_balance)}</strong></article><article className="hud-card"><AlertTriangle/><span>Recurrentes esperados</span><strong>{crc(ctx.recurring_expected)}</strong></article></div>
    <div className="hud-panel deterioration-primary"><span className="strategy-eyebrow">CAUSA PRINCIPAL</span>{primary ? <><h3>{primary.title}</h3><p>{primary.context}</p><div><strong>{labels[primary.code] || primary.code}</strong><span>Actual: {primary.metric.toLocaleString(localeTag(language))} · Comparación: {primary.comparison.toLocaleString(localeTag(language))} {primary.unit}</span></div></> : <><h3>No se detecta deterioro relevante</h3><p>JARVIS no encontró cambios negativos que superen los umbrales definidos con los datos disponibles.</p></>}</div>
