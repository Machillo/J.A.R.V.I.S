...
import { markDincrWelcomeSeen, shouldShowDincrWelcome } from "../lib/firstRunExperience";
import DincrWelcomeStory from "./DincrWelcomeStory";
...
export default function DincrOnboarding({ user, onComplete }) {
  ...
  const [showWelcome, setShowWelcome] = useState(() => shouldShowDincrWelcome(user?.id));
  ...
  const finishWelcome = () => {
    markDincrWelcomeSeen(user?.id);
    setShowWelcome(false);
  };

  if (!profile?.plan_selected && showWelcome) {
    return <DincrWelcomeStory user={profile} onFinish={finishWelcome} />;
  }
