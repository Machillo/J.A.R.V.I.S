...
export default function Finance() {
  const [income, setIncome] = useState([]), [expenses, setExpenses] = useState([]), [ot, setOt] = useState([]);
  ...
  const load = () => Promise.all([getIncome(), getExpenses(), getOvertime()]).then(([a,b,c]) => { setIncome(a); setExpenses(b); setOt(c); });
  useEffect(() => { load(); }, []);

  const submitIncome = async (e) => { e.preventDefault(); const created = await createIncome({ ...incomeForm, amount: Number(incomeForm.amount) }); setIncome(prev => [created, ...prev]); setIncomeForm({ ...incomeForm, amount: "", description: "" }); };
  const submitExpense = async (e) => { e.preventDefault(); const created = await createExpense({ ...expenseForm, amount: Number(expenseForm.amount) }); setExpenses(prev => [created, ...prev]); setExpenseForm({ ...expenseForm, amount: "", description: "" }); };
  const submitOt = async (e) => { e.preventDefault(); const created = await createOvertime({ ...otForm, hours: Number(otForm.hours), hourly_rate: Number(otForm.hourly_rate), multiplier: Number(otForm.multiplier) }); setOt(prev => [created, ...prev]); setOtForm({ ...otForm, hours: "", notes: "" }); };

  return <section><div className="hero"><h1>Ingresos y gastos</h1><p>Registro básico del mes.</p></div><div className="grid3">
