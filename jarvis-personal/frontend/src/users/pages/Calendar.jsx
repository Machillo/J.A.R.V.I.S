...
export default function FinancialCalendar({ plan = "basic" }){
  ...
  return <section className="finva-basic-calendar">
    {plan !== "basic" && <div className="hero"><span>DINCR · {plan === "vip" ? "VIP" : "BASIC"}</span><h1>{tx("Calendario financiero","Financial calendar")}</h1><p>{tx("Pagos, ingresos, deudas y fechas importantes del mes.","Payments, income, debt, and important dates for the month.")}</p></div>}
    <div className="basic-calendar-picker"><input className="month-picker" type="month" aria-label={tx("Mes","Month")} value={period} onChange={e=>setPeriod(e.target.value)}/></div>
      ...
      <div className="basic-calendar-days">
        {Object.keys(grouped).length ? Object.entries(grouped).map(([date,events])=><article className="basic-calendar-day" key={date}>
          ...
          {events.map((event,index)=><div className={`basic-calendar-event ${event.kind}`} key={`${date}-${event.name}-${index}`}>
            <span><b>{event.name}</b><small>{codeLabel("eventKinds", event.kind)}</small></span><strong>{event.amount?money(event.amount):"—"}</strong>
          </div>)}
