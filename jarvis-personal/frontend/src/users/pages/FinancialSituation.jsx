  ...
  return (
    <section className={`mobile-page financial-situation plan-view-${plan}`}>
      <div className="mobile-page-heading">
        ...
        <span>{tx("Actualizá lo que Finva sabe de vos. Cada plan usa un nivel distinto de información.", "Update what Finva knows about you. Each plan uses a different level of information.")}</span>
      </div>
      ...
      <div className="situation-card-list">
        ...
        {plan !== "free" && <SituationCard icon={WalletCards} title={tx("Gastos esenciales", "Essential expenses")} summary={`${money(form.essential_monthly_expenses)} / ${tx("mes", "month")}`} editing={editing === "expenses"} onEdit={() => setEditing(editing === "expenses" ? "" : "expenses")} badge="Basic+">
          ...
          <p className="situation-help">{tx("Finva usa este dato para no recomendar comprometer dinero que necesitás para vivir.", "Finva uses this amount to avoid recommending money that you need for living expenses.")}</p>
        </SituationCard>}
      ...
      {plan === "basic" && <article className="tier-explanation-card"><Sparkles size={20}/><div><strong>{tx("Vista Basic", "Basic view")}</strong><p>{tx("Finva usa ingresos, gastos esenciales, deudas y ahorro para construir recomendaciones determinísticas. Las preferencias avanzadas del Director quedan reservadas para VIP.", "Finva uses income, essential expenses, debt, and savings to build deterministic recommendations. Advanced Director preferences remain exclusive to VIP.")}</p></div></article>}
