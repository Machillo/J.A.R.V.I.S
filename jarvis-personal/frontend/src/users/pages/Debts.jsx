...
export default function Debts() {
  ...
  const submit = async (event) => {
    ...
    const created = await run(() => createDebt({ name: form.name.trim(), remaining_amount: Number(form.remaining_amount), total_amount: optionalNumber(form.total_amount), monthly_payment: optionalNumber(form.monthly_payment), interest_rate: optionalNumber(form.interest_rate), payment_day: optionalNumber(form.payment_day) }));
    if (!created) return;
    setForm(emptyForm); load();
  };
  const pay = async (id) => { const raw = prompt("Monto del pago"); if (!raw) return; const ok = await run(() => payDebt(id, { amount: Number(raw) })); if (ok) load(); };

  return <section><div className="hero"><h1>Deudas</h1><p>Registrá y controlá tus saldos sin mezclar otros workspaces.</p></div>{error && <div className="panel error">{error}</div>}<form className="panel form horizontal" onSubmit={submit}><input required placeholder="Nombre" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}/><input required type="number" min="0" placeholder="Saldo pendiente" value={form.remaining_amount} onChange={(e) => setForm({ ...form, remaining_amount: e.target.value })}/><input type="number" min="0" placeholder="Monto original (opcional)" value={form.total_amount} onChange={(e) => setForm({ ...form, total_amount: e.target.value })}/><input type="number" min="0" placeholder="Cuota mensual (opcional)" value={form.monthly_payment} onChange={(e) => setForm({ ...form, monthly_payment: e.target.value })}/><button>Agregar</button></form><div className="panel table">{rows.length === 0 ? <p>No hay deudas registradas.</p> : rows.map((debt) => <div className="row" key={debt.id}><span><strong>{debt.name}</strong><small>{money(debt.remaining_amount)} pendiente · cuota {money(debt.monthly_payment)} · interés {debt.interest_rate === null ? "sin dato" : `${debt.interest_rate}%`}</small></span><span className="actions"><button onClick={() => pay(debt.id)}>Pagar</button><button className="danger" onClick={async () => { if (confirm("¿Eliminar deuda?")) { const ok = await run(() => deleteDebt(debt.id)); if (ok) load(); } }}>Eliminar</button></span></div>)}</div></section>;
}
