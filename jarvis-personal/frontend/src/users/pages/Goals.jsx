...
export default function Goals({ plan = "free" }) {
  ...
  return <section className="content-first-page">
    <div className="hero"><span>{advanced ? "BASIC 04" : "FREE 05"}</span><h1>{tx("Metas y ahorros", "Goals and savings")}</h1><p>Separá lo que querés alcanzar de lo que planeás guardar cada mes.</p></div>
    ...
    <FinvaFormSheet open={creating} eyebrow="Nueva meta" title={tx("Agregar meta", "Add goal")} onClose={() => setCreating(false)}><form className="form finva-sheet-form" onSubmit={submitGoal}><GoalFields value={form} setValue={setForm} advanced={advanced}/><button className="finva-button finva-button-success">{tx("Guardar meta", "Save goal")}</button></form></FinvaFormSheet>
    <FinvaFormSheet open={Boolean(edit)} eyebrow="Meta" title={tx("Editar meta", "Edit goal")} onClose={() => setEdit(null)}>{edit && <form className="form finva-sheet-form" onSubmit={saveGoal}><GoalFields value={edit} setValue={setEdit} advanced={advanced}/><label><span>Estado</span><select value={edit.status} onChange={(e) => setEdit({...edit,status:e.target.value})}><option value="active">Activa</option><option value="paused">Pausada</option><option value="completed">Completada</option></select></label><button className="finva-button finva-button-primary">{tx("Guardar cambios", "Save changes")}</button></form>}</FinvaFormSheet>
    <FinvaFormSheet open={creatingSavings} eyebrow="Ahorro mensual" title={tx("Programar ahorro", "Schedule savings")} onClose={() => setCreatingSavings(false)}><form className="form finva-sheet-form" onSubmit={submitSavings}><SavingsFields value={savingsForm} setValue={setSavingsForm}/><button className="finva-button finva-button-success">{tx("Guardar programación", "Save schedule")}</button></form></FinvaFormSheet>
    <FinvaFormSheet open={Boolean(editSavings)} eyebrow="Ahorro mensual" title={tx("Editar programación", "Edit schedule")} onClose={() => setEditSavings(null)}>{editSavings && <form className="form finva-sheet-form" onSubmit={saveSavings}><SavingsFields value={editSavings} setValue={setEditSavings}/><label><span>Estado</span><select value={editSavings.status} onChange={(e) => setEditSavings({...editSavings,status:e.target.value})}><option value="active">Activo</option><option value="paused">Pausado</option><option value="completed">Completado</option></select></label><button className="finva-button finva-button-primary">{tx("Guardar cambios", "Save changes")}</button></form>}</FinvaFormSheet>
...
function GoalsList({ rows, advanced, onCreate, onContribute, onEdit, onDelete }) {
  return <><button className="finva-add-strip goal" type="button" onClick={onCreate}><span><Target size={20}/></span><div><strong>{tx("Agregar meta", "Add goal")}</strong><small>Definí un objetivo y medí su progreso</small></div><Plus size={19}/></button><div className="goal-grid content-first-grid">{rows.length ? rows.map((goal) => {
...
function SavingsList({ rows, onCreate, onContribute, onEdit, onDelete }) {
  return <><button className="finva-add-strip savings" type="button" onClick={onCreate}><span><CalendarClock size={20}/></span><div><strong>{tx("Programar ahorro", "Schedule savings")}</strong><small>Elegí cuánto guardar por mes y hasta cuándo</small></div><Plus size={19}/></button><div className="goal-grid content-first-grid">{rows.length ? rows.map((item) => {
