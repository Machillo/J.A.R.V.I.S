...
export default function Goals({ plan = "free", initialView = "goals" }) {
  ...
  const content = !advanced ? freeContent : <section className="content-first-page">
    <div className="hero"><span>{plan === "vip" ? "DINCR · VIP" : advanced ? "DINCR · BASIC" : "DINCR · FREE"}</span><h1>{tx("Metas y ahorros", "Goals and savings")}</h1><p>{tx("Separá lo que querés alcanzar de lo que planeás guardar cada mes.", "Keep your goals separate from what you plan to save each month.")}</p></div>
    <div className="dincr-segmented goals-segmented" role="tablist"><button type="button" className={view === "goals" ? "active" : ""} onClick={() => setView("goals")}><Target size={17}/>{tx("Metas", "Goals")}</button><button type="button" className={view === "savings" ? "active" : ""} onClick={() => setView("savings")}><CalendarClock size={17}/>{tx("Ahorros programados", "Scheduled savings")}</button></div>
  ...
  return <>{content}<DincrFormSheet open={creating} eyebrow={tx("Nueva meta", "New goal")} title={tx("Agregar meta", "Add goal")} onClose={() => setCreating(false)}><form className="form dincr-sheet-form" onSubmit={submitGoal}><GoalFields value={form} setValue={setForm} advanced={advanced}/><button className="dincr-button dincr-button-success">{tx("Guardar meta", "Save goal")}</button></form></DincrFormSheet>
    <DincrFormSheet open={Boolean(edit)} eyebrow={tx("Meta", "Goal")} title={tx("Editar meta", "Edit goal")} onClose={() => setEdit(null)}>{edit && <form className="form dincr-sheet-form" onSubmit={saveGoal}><GoalFields value={edit} setValue={setEdit} advanced={advanced}/><label><span>{tx("Estado", "Status")}</span><select value={edit.status} onChange={(e) => setEdit({...edit,status:e.target.value})}><option value="active">{tx("Activa", "Active")}</option><option value="paused">{tx("Pausada", "Paused")}</option><option value="completed">{tx("Completada", "Completed")}</option></select></label><button className="dincr-button dincr-button-primary">{tx("Guardar cambios", "Save changes")}</button></form>}</DincrFormSheet>
    <DincrFormSheet open={creatingSavings} eyebrow={tx("Ahorro mensual", "Monthly savings")} title={tx("Programar ahorro", "Schedule savings")} onClose={() => setCreatingSavings(false)}><form className="form dincr-sheet-form" onSubmit={submitSavings}><SavingsFields value={savingsForm} setValue={setSavingsForm}/><button className="dincr-button dincr-button-success">{tx("Guardar programación", "Save schedule")}</button></form></DincrFormSheet>
    <DincrFormSheet open={Boolean(editSavings)} eyebrow={tx("Ahorro mensual", "Monthly savings")} title={tx("Editar programación", "Edit schedule")} onClose={() => setEditSavings(null)}>{editSavings && <form className="form dincr-sheet-form" onSubmit={saveSavings}><SavingsFields value={editSavings} setValue={setEditSavings}/><label><span>{tx("Estado", "Status")}</span><select value={editSavings.status} onChange={(e) => setEditSavings({...editSavings,status:e.target.value})}><option value="active">{tx("Activo", "Active")}</option><option value="paused">{tx("Pausado", "Paused")}</option><option value="completed">{tx("Completado", "Completed")}</option></select></label><button className="dincr-button dincr-button-primary">{tx("Guardar cambios", "Save changes")}</button></form>}</DincrFormSheet>
...
function GoalsList({ rows, advanced, onCreate, onContribute, onEdit, onDelete }) {
  return <><button className="dincr-add-strip goal" type="button" onClick={onCreate}><span><Target size={20}/></span><div><strong>{tx("Agregar meta", "Add goal")}</strong><small>{tx("Definí un objetivo y medí su progreso", "Set a target and track its progress")}</small></div><Plus size={19}/></button><div className="goal-grid content-first-grid">{rows.length ? rows.map((goal) => {
