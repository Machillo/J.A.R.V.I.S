...
export default function Goals({ plan = "free", initialView = "goals" }) {
  ...
  const submitGoal = once(async (event) => { event.preventDefault(); if (await run(() => createGoal(goalPayload(form)))) { setForm(goalEmpty); setCreating(false); load(); } });
  const saveGoal = once(async (event) => { event.preventDefault(); if (await run(() => updateGoal(edit.id,goalPayload(edit)))) { setEdit(null); load(); } });
  const submitSavings = once(async (event) => { event.preventDefault(); if (await run(() => createSavingsPlan(savingsPayload(savingsForm)))) { setSavingsForm({...savingsEmpty,start_date:today()}); setCreatingSavings(false); load(); } });
  const saveSavings = once(async (event) => { event.preventDefault(); if (await run(() => updateSavingsPlan(editSavings.id,savingsPayload(editSavings)))) { setEditSavings(null); load(); } });
  ...
  return <>{content}<FinvaFormSheet open={creating} eyebrow={tx("Nueva meta", "New goal")} title={tx("Agregar meta", "Add goal")} onClose={() => setCreating(false)}><form className="form finva-sheet-form" onSubmit={submitGoal}><GoalFields value={form} setValue={setForm} advanced={advanced}/><button className="finva-button finva-button-success">{tx("Guardar meta", "Save goal")}</button></form></FinvaFormSheet>
    ...
    <FinvaFormSheet open={Boolean(editSavings)} eyebrow={tx("Ahorro mensual", "Monthly savings")} title={tx("Editar programación", "Edit schedule")} onClose={() => setEditSavings(null)}>{editSavings && <form className="form finva-sheet-form" onSubmit={saveSavings}><SavingsFields value={editSavings} setValue={setEditSavings}/><label><span>{tx("Estado", "Status")}</span><select value={editSavings.status} onChange={(e) => setEditSavings({...editSavings,status:e.target.value})}><option value="active">{tx("Activo", "Active")}</option><option value="paused">{tx("Pausado", "Paused")}</option><option value="completed">{tx("Completado", "Completed")}</option></select></label><button className="finva-button finva-button-primary" disabled={saving}>{tx("Guardar cambios", "Save changes")}</button></form>}</FinvaFormSheet>
