...
export default function Budget(){
 ...
 <div className="panel budget-list"><h3>{data.is_proposal?"Propuesta inicial":"Tu presupuesto"}</h3>{data.items.map((x,i)=><label key={x.category}><span><b>{x.category}</b><small>Gastado {money(x.spent)} · disponible {money(x.remaining)}</small></span><input type="number" min="0" step="0.01" value={x.monthly_limit} onChange={e=>change(i,e.target.value)}/></label>)}<button className="finva-button finva-button-primary" onClick={save} disabled={saving}>{saving?"Guardando...":"Guardar presupuesto"}</button></div></section>;
}
