...
import { entryCurrencyPayload, entryFormAmount, formatMoney, latestUserRate } from "../../lib/currency";
import AmountCurrencyField, { OriginalAmount } from "../components/AmountCurrencyField";
const language = deviceLanguage();
const tx = (es, en) => language === "es" ? es : en;

// Stored amounts are in the account's base currency.
const money = (value) => formatMoney(value);
// Only manual income and expenses can be typed in another currency.
const manualEntry = (row) => row?.origin === "salary" || row?.origin === "expense";
...
export default function Transactions({ plan = "free" }) {
  ...
  const save = async (event) => {
    ...
    try {
      await updateFreeMovement(edit.movement_id, { transaction_date: edit.transaction_date, description: edit.description, amount: Number(edit.amount), transaction_type: edit.transaction_type, category: categoryValue(edit.category), notes: edit.notes || "", ...(manualEntry(edit) ? entryCurrencyPayload(edit) : {}) });
      setEdit(null); load();
