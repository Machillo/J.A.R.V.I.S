    ...
    {chatOpen && <div className="support-chat-overlay" role="dialog" aria-modal="true" aria-label={tx("Chat de soporte", "Support chat")}>
      <div className="support-chat-shell">
        ...
        <section className="support-conversation" aria-live="polite">
          ...
          {needsText && <div className="support-composer"><textarea rows="2" autoFocus aria-label={tx("Tu respuesta para soporte", "Your answer for support")} value={draft} maxLength={step === "subject" ? 140 : 1000} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && canSendText) { event.preventDefault(); answer(); } }} placeholder={tx("Escribí tu respuesta…", "Type your answer…")}/><button type="button" disabled={!canSendText} onClick={() => answer()} aria-label={tx("Enviar respuesta", "Send answer")}><Send size={19}/></button>{(step === "expected" || (step === "screen" && form.category === "improvement")) && <button className="support-skip" type="button" onClick={() => answer("")}>{tx("Omitir", "Skip")}</button>}</div>}
