...
-- Objects that already exist are left exactly as they are.
--
-- Preflight (read-only):
--   SELECT t, to_regclass('public.' || t) IS NOT NULL AS present
--   FROM unnest(ARRAY['store_subscriptions','store_subscription_events',
--     'finva_budget_items','finva_recurring_items','finva_goal_contributions',
--     'accounts','workspaces','financial_goals','features','plans','plan_features']) t;
--   -- accounts, workspaces, financial_goals, features, plans, plan_features must be present.
...
-- 2. Notification job index the runtime DDL created (schema.sql only).
...
-- 4. Basic feature catalogue. Existing rows and grants are never changed
...
--                   'finva_recurring_items','finva_goal_contributions']) t
-- WHERE to_regclass('public.' || t) IS NULL
--    OR NOT (SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.' || t))
--    OR has_table_privilege('anon', to_regclass('public.' || t), 'SELECT')
